...
class KeypleRemoteService(
    ...
    suspend fun <T, R> executeRemoteService(
        ...
        try {
            ...
            while (serverResponse.action != END_REMOTE_SERVICE) {
                ...
                val deviceAnswer = when (service) {
                    IS_CONTACTLESS ->
                        messageProcessor.isContactless()

                    IS_CARD_PRESENT ->
                        messageProcessor.isCardPresent()

                    TRANSMIT_CARD_SELECTION_REQUESTS ->
                        transmitCardSelectionRequests(serverResponse)

                    TRANSMIT_CARD_REQUEST ->
                        transmitCardRequest(serverResponse)
    ...
    private suspend fun transmitCardRequest(message: MessageDTO): String {
        ...
        try {
            cardResponse = processCardRequest(
                cardRequest,
                transmitCardRequestsCmdBody.parameters.channelControl
            )
    ...
    private suspend fun transmitCardSelectionRequests(message: MessageDTO): String {
        ...
        var error: Error? = null
        val nbIterations =
            transmitCardSelectionRequestsCmdBody.parameters.cardSelectionRequests.size

        for (i in 0..<nbIterations) {
            try {
                val cardSelectionResponse = processCardSelectionRequest(
        ...
        val transmitCardSelectionRequestsRespBody = if (error == null) {
    ...
    private suspend fun processCardSelectionRequest(
        ...
        val cardSelectionResponse = CardSelectionResponse(
            hasMatched = cardSelectionRequest.successfulSelectionStatusWords.contains(
